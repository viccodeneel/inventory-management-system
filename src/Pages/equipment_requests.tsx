    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        const stats = {
          ...
          pending: result.data.filter((req: EquipmentRequest) => req.status === 'pending').length,
          approved: result.data.filter((req: EquipmentRequest) => req.status === 'approved').length,
          rejected: result.data.filter((req: EquipmentRequest) => req.status === 'rejected').length,
          returned: result.data.filter((req: EquipmentRequest) => req.status === 'returned').length,
        };
      ...
      setRequests([]);
      setRequestStats({ total: 0, pending: 0, approved: 0, rejected: 0, returned: 0 });
    }
  };
    ...
    try {
      ...
      if (result.success && result.data) {
        setRequests(prev => prev.map(req => 
          req.id === selectedRequest.id ? result.data! : req
        ));
        ...
        alert(`Request approved successfully! ${result.data.approval_code ? `Approval code: ${result.data.approval_code}` : ''}`);
      } else {
                ...
                <table className="requests-table">
                  <thead>
                    <tr>
                      ...
                      <th>Status</th>
                      {(activeTab === 'pending' || activeTab === 'approved') && <th>Actions</th>}
                    </tr>
                  </thead>
                  <tbody>
                    {currentRequests.length > 0 ? (
                      ...
                      <tr>
                        <td colSpan={activeTab === 'pending' ? 9 : activeTab === 'approved' ? 9 : activeTab === 'returned' ? 10 : 8} style={{ textAlign: 'center', padding: '20px' }}>
